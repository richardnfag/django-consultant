...
class SubmitProposalView(APIView):
    def post(self, request, format=None):
        try:
            ...
            proposal = LoanProposal.objects.create(
                **static_fields, approved="pending", auto_approved="pending"
            )

            for field in custom_fields:
                loan_field = LoanField.objects.get(name=field["name"])
                value = field["value"]
                proposal.custom_fields.add(
                    loan_field, through_defaults={"value": value}
                )
